...
function createServerInfoService({ client, dataDir, embedColor = 0x5b2a86 }) {
  ...
  function buildEmbeds(data = getData()) {
    ...
    const compactChannelLinks = [
      data.channels?.rules ? `📜 <#${data.channels.rules}>` : null,
      data.channels?.support ? `🛟 <#${data.channels.support}>` : null,
      data.channels?.tickets ? `🎫 <#${data.channels.tickets}>` : null,
      data.channels?.announcements
        ? `📢 <#${data.channels.announcements}>`
        : null,
      data.channels?.status ? `📡 <#${data.channels.status}>` : null,
    ]
      .filter(Boolean)
      .join('  •  ');
    ...
    const featuresEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          ...
          value:
            compactChannelLinks ||
            'No Discord channels have been configured yet.',
          inline: false,
        }
      )
      .setFooter({
